...
struct TarHeader {
    ...
    static TYPELAGS getTypeFlag(const DWORD mode) noexcept;
...
inline TarHeader::TYPELAGS TarHeader::getTypeFlag(const DWORD mode) noexcept {
    if(mode == INVALID_FILE_ATTRIBUTES) return TarHeader::TYPELAGS::AREGTYPE;

    if(mode & FILE_ATTRIBUTE_DIRECTORY) return TarHeader::TYPELAGS::DIRTYPE;
    if(mode & FILE_ATTRIBUTE_REPARSE_POINT) return TarHeader::TYPELAGS::SYMTYPE;

    return TarHeader::TYPELAGS::REGTYPE;
}
